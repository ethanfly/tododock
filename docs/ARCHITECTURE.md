...
Markdown 编辑器是延迟加载的受控组件。视觉模式通过安全 Markdown 渲染生成可编辑 DOM，输入后立即序列化回 CommonMark/GFM；源码模式直接编辑同一字符串。剪贴板与拖放接收纯文本或本机图片；原始 HTML 不进入编辑 DOM。粘贴的栅格图片压缩为 `data:` URL 写入 Markdown，远程图片仍只显示占位。视觉层不保存专有 JSON，因此导出、备份与未来迁移仍只处理 Markdown。可选的大模型调用走 Rust（OpenAI 兼容 Chat Completions），密钥不离开本机设置，仅在用户主动根据图片生成待办时发送图片。
...
- 新建、设置和编辑使用独立 Tauri 窗口（`#/create`、`#/settings`、`#/edit/<id>`）；主窗口列表不再弹出这些模态框。任一辅助窗口打开时，主窗口不自动隐藏。菜单、关闭到托盘说明和提醒交互仍抑制隐藏。
