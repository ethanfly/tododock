...
同日已对照官方 `actions/runner-images` 清单核对六个 CI 标签和架构；配置标签均存在，Linux workflow 同时显式安装 pkg-config、DBus、WebKit、托盘和 SVG 构建依赖。但 Windows 本机之外仍需在目标仓库实际触发 CI 后保存每个平台的构建日志和 manifest。
