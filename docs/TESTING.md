...
- 移除启动时强制注册后，进程级全局快捷键仍能从 Chrome 唤起隐藏窗口并把输入框置于可输入状态；注册冲突会降级为设置页提示，不再阻止应用启动。
- 在设置中关闭全局快捷键后，从 Chrome 按 `Ctrl+Shift+Space` 不再唤起隐藏窗口；重新启用并保存后，同一按键再次成功唤起。
