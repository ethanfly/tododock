...
export function TodoEditor({ todo, defaultReminderMinutes, onClose, onSave }: TodoEditorProps) {
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="editor-panel" role="dialog" aria-modal="true" aria-labelledby={dialogTitleId}>
        <form
          onSubmit={submit}
          onKeyDown={(event) => {
            if (event.key === "Tab") {
        ...
        >
          <header className="editor-header">
            <span id={dialogTitleId} className="eyebrow">编辑任务</span>
            <IconButton label="关闭编辑" onClick={onClose}><X size={17} /></IconButton>
          </header>
